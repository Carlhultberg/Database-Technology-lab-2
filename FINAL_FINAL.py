...
class Booking_Details():
    def __init__(self, master):
        ...
        # WRONG
        #sql = "SELECT room_id, room_name, room_facilities FROM resources WHERE available = 0"
        #sql = "SELECT resources.room_id, resources.room_name, resources.room_facilities FROM resources INNER JOIN \
        #current_bookings ON resources.room_id = current_bookings.room_id WHERE current_bookings.booking_timing != (%s) \
        #AND current_bookings.booking_date != (%s) GROUP BY resources.room_id"

        sql = "SELECT resources.room_id, resources.room_name, resources.room_facilities FROM resources WHERE \
        resources.room_id NOT IN (SELECT current_bookings.room_id FROM current_bookings WHERE \
        current_bookings.booking_timing = (%s) AND current_bookings.booking_date = (%s))"
    ...
    def choose_room(self):
        self.selections = self.available_rooms.curselection()
        self.value = self.available_rooms.get(self.selections)
        self.selections = [int(x) + 1 for x in self.selections]
        print("room index:", self.selections, ": '%s'" % str(self.value))
        ...
        update_rooms = "UPDATE current_bookings SET room_id = %s WHERE booking_id = %s"
        cur.execute(update_rooms, (self.value, id))
        conn.commit()
...
class Select_Participants():
    def __init__(self, master):
        ...
        cur.execute("SELECT * FROM person WHERE staff_id != 1000")
    ...
    def choose_participants(self):
        self.participants_selections = self.available_persons.curselection()
        self.participants_selections = [int(x) + 1 for x in self.participants_selections]
        print("participants id:", self.participants_selections)

        cur.execute("SELECT fname, lname FROM person WHERE staff_id = ANY(%s) AND staff_id != 1000", (self.participants_selections,))
        conn.commit()
        ...
        cur.execute("UPDATE current_bookings SET participants = %s WHERE booking_id = %s",
                    (self.participants_names[:], (id)))
        conn.commit()

        cur.execute(sql.SQL("DELETE FROM current_bookings WHERE participants LIKE NULL"))
        #cur.execute(sql.SQL("DELETE FROM current_bookings WHERE team_id = 1000 OR staff_id = 1000 OR room_id = 1000"))
        conn.commit()

        window = Booking_View(self.master)
        self.hide()
        window.show()
...
class Show_Times():
    ...
    def choose_time(self):
        ...
        cur.execute("SELECT room_id FROM current_bookings ORDER BY booking_id ASC")
        self.room_id = cur.fetchall()[-1]
        room_id = str(self.room_id[0])
...
class Show_Dates():
    def __init__(self, master):
        ...
        self.label_available_dates = Label(self.frame,
                                           text="When do you need a room? Specify date to proceed with booking.")
    ...
    def choose_date(self):
        ...
        cur.execute("INSERT INTO current_bookings VALUES (%s,%s, %s, %s, %s, %s, %s)", (
        self.id, self.start_time, self.start_date, self.staff_id, self.team_id, self.room_id, self.participants))
        conn.commit()
...
class Personal_Details_Remove():
    def __init__(self, master):
        ...
        cur.execute("SELECT fname, lname, staff_id FROM person WHERE staff_id != 1000")
        self.person = cur.fetchall()
    ...
    def choose_name(self):
        self.name_selections = self.all_persons.curselection()
        self.person_id = self.all_persons.get(self.name_selections[0])
        self.name_selections = [int(x) + 1 for x in self.name_selections]
        # print("person index:", selections, ": '%s'" % person_id[2])
        person_id = self.person_id[2]
    ...
    def choose_time(self):
        ...
        self.start_date = self.person_times.get(self.selections[0])
        self.selections = [int(x) + 1 for x in self.selections]
        # print("time index:", self.selections, ": '%s'" % self.start_time[1])
        self.start_time = str(self.start_time[1])
        self.start_date = str(self.start_date[2])

        cur.execute(
            sql.SQL("DELETE FROM current_bookings WHERE booking_timing = (%s) \
            AND CURRENT_DATE < booking_date"), [self.start_time])
        conn.commit()

        window = Deletion_View(self.master)
        self.hide()
        window.show()
...
class Booking_View():
    def __init__(self, master):
        ...
        self.frame = Frame(master, width=55, height=10)
        master.wm_title("")
        self.label_available_times = Label(self.frame, text="your meeeting has succesfully been booked!")
...
class Deletion_View():
    def __init__(self, master):
        ...
        self.frame = Frame(master, width=55, height=10)
        master.wm_title("")
        self.label_available_times = Label(self.frame, text="If your booking was at least one day a head, \n it has been deleted with no cost. Otherwise your team has been charged.")
